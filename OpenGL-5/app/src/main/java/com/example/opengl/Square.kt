...
class Square {
    ...
    private val mProgram: Int       // 存儲OpenGL ES程式的引用
    ...
    init {
        ...
        mProgram = GLShaderFactory.defaultShader //著色器代碼
    }
    ...
    private fun initVertexBuffer() {
        ...
        vertexBuffer!!.position(0)      // 使用position()方法來重置緩衝區的位置
    }
    ...
    private fun initDrawListBuffer() {
        ...
        drawListBuffer!!.position(0)    // 使用position()方法來重置緩衝區的位置
    }
    ...
    fun draw() {
        ...
        // 獲取變數位置
        val positionHandle = GLES20.glGetAttribLocation(mProgram, GLShaderFactory.FIELD_POSITION)       // 使用glGetAttribLocation()方法獲取Java代碼中的position變數的控制碼
        val colorHandle = GLES20.glGetUniformLocation(mProgram, GLShaderFactory.FIELD_COLOR)    // 使用glGetUniformLocation()方法獲取matrix變數的控制碼
        val mMVPMatrixHandle = GLES20.glGetUniformLocation(mProgram, GLShaderFactory.FIELD_MATRIX_MVP)  //使用glGetUniformLocation()方法獲取matrix變數的控制碼

        GLES20.glEnableVertexAttribArray(positionHandle)    // 使用glEnableVertexAttribArray()方法啟用控制碼
        // 使用glVertexAttribPointer()方法來將position控制碼指向我們的頂點緩衝區
        // (Input: 頂點緩衝區, 每個頂點的座標數, 座標的類型, 每個頂點的位元組偏移量)
        ...
        //
        GLES20.glUniformMatrix4fv(mMVPMatrixHandle, 1, false, mMVPMatrix, 0)    // 使用glUniformMatrix()方法將其指向產品矩陣。
    ...
    private fun drawTriangles() {
        if (mDrawMode == GLES20.GL_TRIANGLE_FAN || mDrawMode == GLES20.GL_TRIANGLE_STRIP || mDrawMode == GLES20.GL_TRIANGLES)
            ...
            // glDrawElements()方法讓你使用faces緩衝區來創建三角形。需要的參數有，頂點索引的總數，每個索引的類型和面緩衝區
